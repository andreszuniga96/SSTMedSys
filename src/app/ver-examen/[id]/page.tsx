...
// Icono de WhatsApp compartido por los dos botones de la página
...
export default function VerExamenPage() {
    ...
    // Compartir el certificado por WhatsApp con el enlace de verificación pre-armado
    const compartirWhatsApp = () => {
        const nombre = pac.nombre_completo || "Trabajador";
        const fecha = ev?.fecha_actual
            ? new Date(ev.fecha_actual).toLocaleDateString("es-CO", { year: "numeric", month: "long", day: "numeric" })
            : "";
        const tieneConcepto = Boolean(cert?.concepto_medico);
        // Mensaje adaptativo: si aún no hay certificado emitido, no afirma un concepto
        const mensaje = tieneConcepto
            ? `Hola, le comparto mi Certificado Médico Ocupacional (SST MedSys).\n` +
              `Paciente: ${nombre}\n` +
              `Concepto: ${cert.concepto_medico}${fecha ? ` · ${fecha}` : ""}\n` +
              `Verifíquelo en línea aquí: ${window.location.href}`
            : `Hola, le comparto el enlace de mi examen médico ocupacional (SST MedSys).\n` +
              `Paciente: ${nombre}\n` +
              `Consúltelo en línea aquí: ${window.location.href}`;
        window.open(
            ...
            <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
                        ...
                        <div className="px-6 py-4 bg-gradient-to-r from-teal-900 to-cyan-900 flex items-center justify-between flex-wrap gap-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white transition-all shadow"
                                    style={{ background: "#25D366", boxShadow: "0 2px 10px rgb(37 211 102 / 0.35)" }}
                                    title="Compartir el certificado por WhatsApp"
                                >
                                    <IconoWhatsApp />
                                    WhatsApp
                                </button>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col md:flex-row items-center justify-between gap-6 animate-fade-in">
                    ...
                    <div className="flex flex-wrap gap-3 justify-center">
                        ...
                        <button
                            onClick={compartirWhatsApp}
                            className="inline-flex items-center gap-2 px-5 py-2.5 font-semibold rounded-xl text-sm transition-all text-white shadow-md"
                            style={{ background: "#25D366", boxShadow: "0 2px 10px rgb(37 211 102 / 0.35)" }}
                        >
                            <IconoWhatsApp />
